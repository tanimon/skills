...
同じ原則により、次の YAML・Markdown として妥当な表記も LF・コメントなしの正準形と同じ値として解釈する: CRLF 改行(lint は `line-ending` で LF への正規化を提案する)、スカラー値のインラインコメント(`type: note # 補足` の値は `note`。YAML の規則どおり `#` の直前に空白がある場合のみコメントとみなし、`C#` のような値や引用符内の `#` は値の一部として扱う)。
