...
`wiki-lint.sh` で機械チェックを行う(正準形一致の検証 → 正準形を前提とした意味チェックの二段構え。重大度は ERROR / SUGGEST の2段階)。
...
- `references/operations.md` — Init / Ingest / Query / Lint の実行手順の詳細と、`wiki-lint.sh` の check id(ERROR / SUGGEST)一覧。
