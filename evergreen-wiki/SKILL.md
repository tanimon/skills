...
`wiki-lint.sh` で機械チェックを行う(正準形一致の検証 → 正準形を前提とした意味チェックの二段構え。ERROR 13種 + SUGGEST 4種)。
...
- `references/operations.md` — Init / Ingest / Query / Lint の実行手順の詳細と、`wiki-lint.sh` の check id(ERROR 13種 + SUGGEST 4種)一覧。
